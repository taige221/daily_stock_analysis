...
from src.schemas.theme_event import ThemeDefinitionSchema
from src.services.theme_stock_pool_service import ThemeStockPoolService, canonicalize_stock_code
...
class ThemeBoardResolverService:
    ...
    def __init__(self):
        ...
        self.cache_path = repo_root / "data" / "theme_board_cache.json"
        self.global_mapping_path = repo_root / "data" / "theme_board_mappings.json"
        self.example_global_mapping_path = repo_root / "src" / "data" / "theme_board_mappings.example.json"
    ...
    def _get_tushare_client(self) -> Optional[_TushareHttpClient]:
        ...
        token = str(get_config().tushare_token or "").strip()
        if not token:
            return None
    ...
    def _is_cache_usable(self, cache_payload: object) -> bool:
        ttl = int(get_config().theme_board_cache_ttl_seconds)
        if ttl <= 0:
            return False
