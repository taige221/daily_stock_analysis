...
from src.storage import DatabaseManager, ThemePickerTaskHistory
...
class ThemePickerTaskService:
    ...
    def _persist_task(self, task: ThemePickerTaskInfo) -> None:
        ...
        try:
            self._db.save_theme_picker_task_history(
    ...
    def _cleanup_persisted_history(self) -> None:
        try:
            removed = self._db.cleanup_theme_picker_task_history(
    ...
    def _recover_unfinished_tasks(self) -> None:
        try:
            records = self._db.list_theme_picker_task_history_by_statuses(
                ['pending', 'processing'],
    ...
    def retry_task(self, task_id: str) -> ThemePickerTaskInfo:
        persisted = self._db.get_theme_picker_task_history(task_id)
    ...
    def list_tasks(self, limit: int = 20) -> list[ThemePickerTaskInfo]:
        ...
        for persisted in self._db.list_theme_picker_task_history(limit=max(1, limit) * 3):
